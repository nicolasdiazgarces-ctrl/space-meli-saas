...
export default async function handler(req, res) {
  ...
  // ── Rate limit: max 10 imágenes por IP por minuto (anti DDoS / credit drain) ─
  const ip = getClientIP(req);
  if (isRateLimited(ip, 'img', 10, 60_000)) {
    return res.status(429).json({ error: 'Demasiadas solicitudes. Esperá un minuto.' });
  }

  const { key, prompt, imageB64 } = req.body || {};

  // fix #7: sanitize inputs before use
  const cleanPrompt = sanitizeString(prompt, 2000);
  if (!key || !cleanPrompt) return res.status(400).json({ error: 'Missing key or prompt' });

  // fix #2: imageB64 size bomb check (2,800,000 chars ≈ 1.5 MB image)
  ...
  if (!togetherKey)           return res.status(500).json({ error: 'Error interno del servidor' });
  ...
  try {
    ...
    if (!dbRes.ok) {
      console.error('[SpaceMELI/generate-image] Supabase error:', dbRes.status);
    ...
    if (imageB64) {
      ...
      payload = {
        ...
        prompt: cleanPrompt,
        n: 1, steps: 4, width: 1024, height: 1024,
        response_format: 'b64_json'
      };
    }
    ...
    if (!imgRes.ok || imgData.error) {
      ...
      console.error('[SpaceMELI/generate-image] Together AI error:', imgData);
      return res.status(502).json({ error: 'Error interno del servidor' });
    }

    const b64 = imgData.data?.[0]?.b64_json;
    const url = imgData.data?.[0]?.url;
    if (!b64 && !url) return res.status(502).json({ error: 'Error interno del servidor' });
    ...
    // fix #8: log real error, generic message to client
    console.error('[SpaceMELI/generate-image] Error:', err);
